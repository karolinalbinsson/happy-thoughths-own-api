...
export const App = () => {
	...
	const getThoughts = () => {
		fetch("https://karolin-happy-thoughts.herokuapp.com/thoughts")
			.then((res) => res.json())
			.then((data) => {
				setThoughts(data);
				setLoading(false);
	...
	const postThought = (message) => {
		...
		fetch(
			"https://karolin-happy-thoughts.herokuapp.com/thoughts",
			requestOptions
		)
			.then((res) => res.json())
			.then((newThought) => {
	...
	return (
		<section className="app-container">
			...
			{!messageOK && (
				<p>Couldn't save message, check the length (5-140) and try again.</p>
			)}
			{loading && <Loader />}
			{!loading && (
